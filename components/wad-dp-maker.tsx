...
const FRAME_CONFIG = {
  id: "wad-doctor-dp-2026",
  src: "/frames/wad-doctor-dp-frame.png",
  outputSize: 900,
  photoX: 64,
  photoY: 64,
  photoWidth: 772,
...
function buildFileName() {
  return "act-on-asthma-wad-2026-doctor-dp.jpg";
}
...
export function WadDpMaker() {
  ...
  useEffect(() => {
    ...
    void loadImage(FRAME_CONFIG.src)
      ...
      .catch(() => {
        if (isActive) {
          setErrorMessage("Unable to load the WAD frame.");
        }
      });
  ...
  async function handleGenerate() {
    if (!photoImage || !frameImage || !croppedAreaPixels) {
      setErrorMessage("Upload and position a photo before generating the DP.");
      return;
    }
      ...
      try {
        ...
        const message =
          loggingError instanceof Error
            ...
            : "The image was created, but the count could not be saved.";
  ...
  return (
    <main className="min-h-screen bg-[var(--page-bg)] px-3 py-3 text-[var(--ink)] sm:px-4 sm:py-4 lg:px-8 lg:py-8">
      <div className="mx-auto grid max-w-7xl gap-4 lg:grid-cols-[360px_minmax(0,1fr)] xl:grid-cols-[400px_minmax(0,1fr)]">
        ...
        <section className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_360px]">
          <div className="rounded-2xl border border-[var(--panel-border)] bg-[var(--panel)] p-4 shadow-[0_18px_60px_rgba(12,26,35,0.10)] sm:p-5">
            ...
            {statusMessage ? (
              <div className="mt-4 rounded-lg border border-[var(--panel-border)] bg-white px-4 py-3 text-sm text-[var(--muted-ink)]">
                {statusMessage}
              </div>
            ) : null}
          </div>
